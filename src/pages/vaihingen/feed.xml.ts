// src/pages/vaihingen/feed.xml.ts
// Atom 1.0 feed for Vaihingen an der Enz events — app-backbone ready.
...
const FEED_URL = `${SITE}/${vaihingen}/feed.xml`;
const CITY_URL = `${SITE}/${vaihingen}/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url        = `${SITE}/${vaihingen}/events/${esc(ev.slug)}/`;
    ...
    const ogImage    = `${SITE}/og/${vaihingen}/${esc(ev.slug)}.jpg`;
...
<feed xmlns="http://www.w3.org/2005/Atom"
      ...
      xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Vaihingen an der Enz | rausgucken.de</title>
  <subtitle>Aktuelle Events, Workshops und Ausstellungen in Vaihingen an der Enz – täglich aktualisiert.</subtitle>
