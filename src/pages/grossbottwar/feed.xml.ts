// src/pages/grossbottwar/feed.xml.ts
// Atom 1.0 feed for Großbottwar events — app-backbone ready.
...
import eventsRaw from "../../../public/data/grossbottwar/events-current.json";

const SITE     = "https://www.rausgucken.de";
const grossbottwar  = "grossbottwar";
const FEED_URL = `${SITE}/${grossbottwar}/feed.xml`;
const CITY_URL = `${SITE}/${grossbottwar}/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url        = `${SITE}/${grossbottwar}/events/${esc(ev.slug)}/`;
    ...
    const ogImage    = `${SITE}/og/${grossbottwar}/${esc(ev.slug)}.jpg`;
...
<feed xmlns="http://www.w3.org/2005/Atom"
      ...
      xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Großbottwar | rausgucken.de</title>
  <subtitle>Aktuelle Events, Workshops und Ausstellungen in Großbottwar – täglich aktualisiert.</subtitle>
