// src/pages/bietigheim/feed.xml.ts
// Atom 1.0 feed for Bietigheim events.

import eventsRaw from "../../../public/data/bietigheim/events-current.json";

const SITE = "https://www.rausgucken.de";
const FEED_URL = `${SITE}/bietigheim/feed.xml`;
const CITY_URL = `${SITE}/bietigheim/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url = `${SITE}/bietigheim/events/${esc(ev.slug)}/`;
...
<feed xmlns="http://www.w3.org/2005/Atom" xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Bietigheim-Bissingen | rausgucken.de</title>
  <subtitle>Aktuelle Events und Veranstaltungen in Bietigheim-Bissingen – wöchentlich aktualisiert.</subtitle>
