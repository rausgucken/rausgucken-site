// src/pages/affalterbach/feed.xml.ts
// Atom 1.0 feed for Affalterbach events — app-backbone ready.
...
import eventsRaw from "../../../public/data/affalterbach/events-current.json";

const SITE     = "https://www.rausgucken.de";
const affalterbach  = "affalterbach";
const FEED_URL = `${SITE}/${affalterbach}/feed.xml`;
const CITY_URL = `${SITE}/${affalterbach}/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url        = `${SITE}/${affalterbach}/events/${esc(ev.slug)}/`;
    ...
    const ogImage    = `${SITE}/og/${affalterbach}/${esc(ev.slug)}.jpg`;
...
<feed xmlns="http://www.w3.org/2005/Atom"
      ...
      xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Affalterbach | rausgucken.de</title>
  <subtitle>Aktuelle Events, Workshops und Ausstellungen in Affalterbach – täglich aktualisiert.</subtitle>
