// src/pages/steinheim/feed.xml.ts
// Atom 1.0 feed for Steinheim an der Murr events — app-backbone ready.
...
import eventsRaw from "../../../public/data/steinheim/events-current.json";

const SITE     = "https://www.rausgucken.de";
const steinheim  = "steinheim";
const FEED_URL = `${SITE}/${steinheim}/feed.xml`;
const CITY_URL = `${SITE}/${steinheim}/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url        = `${SITE}/${steinheim}/events/${esc(ev.slug)}/`;
    ...
    const ogImage    = `${SITE}/og/${steinheim}/${esc(ev.slug)}.jpg`;
...
<feed xmlns="http://www.w3.org/2005/Atom"
      ...
      xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Steinheim an der Murr | rausgucken.de</title>
  <subtitle>Aktuelle Events, Workshops und Ausstellungen in Steinheim an der Murr – täglich aktualisiert.</subtitle>
