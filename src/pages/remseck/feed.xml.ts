// src/pages/remseck/feed.xml.ts
// Atom 1.0 feed for Remseck am Neckar events.

import eventsRaw from "../../../public/data/remseck/events-current.json";

const SITE = "https://www.rausgucken.de";
const FEED_URL = `${SITE}/remseck/feed.xml`;
const CITY_URL = `${SITE}/remseck/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url = `${SITE}/remseck/events/${esc(ev.slug)}/`;
...
<feed xmlns="http://www.w3.org/2005/Atom" xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Remseck am Neckar | rausgucken.de</title>
  <subtitle>Aktuelle Events und Veranstaltungen in Remseck am Neckar – wöchentlich aktualisiert.</subtitle>
