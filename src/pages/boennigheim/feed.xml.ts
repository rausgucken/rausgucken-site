// src/pages/boennigheim/feed.xml.ts
// Atom 1.0 feed for Bönnigheim events — app-backbone ready.
...
const FEED_URL = `${SITE}/${boennigheim}/feed.xml`;
const CITY_URL = `${SITE}/${boennigheim}/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url        = `${SITE}/${boennigheim}/events/${esc(ev.slug)}/`;
    ...
    const ogImage    = `${SITE}/og/${boennigheim}/${esc(ev.slug)}.jpg`;
...
<feed xmlns="http://www.w3.org/2005/Atom"
      ...
      xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Bönnigheim | rausgucken.de</title>
  <subtitle>Aktuelle Events, Workshops und Ausstellungen in Bönnigheim – täglich aktualisiert.</subtitle>
