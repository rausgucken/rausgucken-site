// src/pages/ditzingen/feed.xml.ts
// Atom 1.0 feed for Ditzingen events — app-backbone ready.
...
import eventsRaw from "../../../public/data/ditzingen/events-current.json";

const SITE     = "https://www.rausgucken.de";
const ditzingen  = "ditzingen";
const FEED_URL = `${SITE}/${ditzingen}/feed.xml`;
const CITY_URL = `${SITE}/${ditzingen}/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url        = `${SITE}/${ditzingen}/events/${esc(ev.slug)}/`;
    ...
    const ogImage    = `${SITE}/og/${ditzingen}/${esc(ev.slug)}.jpg`;
...
<feed xmlns="http://www.w3.org/2005/Atom"
      ...
      xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Ditzingen | rausgucken.de</title>
  <subtitle>Aktuelle Events, Workshops und Ausstellungen in Ditzingen – täglich aktualisiert.</subtitle>
