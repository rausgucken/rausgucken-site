// src/pages/schwieberdingen/feed.xml.ts
// Atom 1.0 feed for Schwieberdingen events — app-backbone ready.
...
import eventsRaw from "../../../public/data/schwieberdingen/events-current.json";

const SITE     = "https://www.rausgucken.de";
const schwieberdingen  = "schwieberdingen";
const FEED_URL = `${SITE}/${schwieberdingen}/feed.xml`;
const CITY_URL = `${SITE}/${schwieberdingen}/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url        = `${SITE}/${schwieberdingen}/events/${esc(ev.slug)}/`;
    ...
    const ogImage    = `${SITE}/og/${schwieberdingen}/${esc(ev.slug)}.jpg`;
    ...
    ${categoryTags}
    <rg:city>${schwieberdingen}</rg:city>
    ${rgPrice}
    ${rgAgeMin}
    ${rgAgeMax}
...
<feed xmlns="http://www.w3.org/2005/Atom"
      ...
      xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Schwieberdingen | rausgucken.de</title>
  <subtitle>Aktuelle Events, Workshops und Ausstellungen in Schwieberdingen – täglich aktualisiert.</subtitle>
