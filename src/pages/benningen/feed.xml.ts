// src/pages/benningen/feed.xml.ts
// Atom 1.0 feed for Benningen am Neckar events — app-backbone ready.
...
const FEED_URL = `${SITE}/${benningen}/feed.xml`;
const CITY_URL = `${SITE}/${benningen}/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url        = `${SITE}/${benningen}/events/${esc(ev.slug)}/`;
    ...
    const ogImage    = `${SITE}/og/${benningen}/${esc(ev.slug)}.jpg`;
...
<feed xmlns="http://www.w3.org/2005/Atom"
      ...
      xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Benningen am Neckar | rausgucken.de</title>
  <subtitle>Aktuelle Events, Workshops und Ausstellungen in Benningen am Neckar – täglich aktualisiert.</subtitle>
