// src/pages/tamm/feed.xml.ts
// Atom 1.0 feed for Tamm events.

import eventsRaw from "../../../public/data/tamm/events-current.json";

const SITE = "https://www.rausgucken.de";
const FEED_URL = `${SITE}/tamm/feed.xml`;
const CITY_URL = `${SITE}/tamm/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url = `${SITE}/tamm/events/${esc(ev.slug)}/`;
...
<feed xmlns="http://www.w3.org/2005/Atom" xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Tamm | rausgucken.de</title>
  <subtitle>Aktuelle Events und Veranstaltungen in Tamm – wöchentlich aktualisiert.</subtitle>
