// src/pages/freiberg/feed.xml.ts
import eventsRaw from "../../../public/data/freiberg/events-current.json";

const SITE     = "https://www.rausgucken.de";
const FEED_URL = `${SITE}/freiberg/feed.xml`;
const CITY_URL = `${SITE}/freiberg/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url        = `${SITE}/freiberg/events/${esc(ev.slug)}/`;
    ...
    const dateLabel  = ev.date_start
      ? new Date(ev.date_start).toLocaleDateString("de-DE", { weekday: "long", day: "numeric", month: "long", year: "numeric" })
      : "";
...
<feed xmlns="http://www.w3.org/2005/Atom" xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Freiberg am Neckar | rausgucken.de</title>
  <subtitle>Aktuelle Events und Veranstaltungen in Freiberg am Neckar – täglich aktualisiert.</subtitle>
