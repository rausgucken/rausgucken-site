// src/pages/gerlingen/feed.xml.ts
// Atom 1.0 feed for Gerlingen events — app-backbone ready.
...
const FEED_URL = `${SITE}/${gerlingen}/feed.xml`;
const CITY_URL = `${SITE}/${gerlingen}/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url        = `${SITE}/${gerlingen}/events/${esc(ev.slug)}/`;
    ...
    const ogImage    = `${SITE}/og/${gerlingen}/${esc(ev.slug)}.jpg`;
...
<feed xmlns="http://www.w3.org/2005/Atom"
      ...
      xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Gerlingen | rausgucken.de</title>
  <subtitle>Aktuelle Events, Workshops und Ausstellungen in Gerlingen – täglich aktualisiert.</subtitle>
