// src/pages/besigheim/feed.xml.ts
// Atom 1.0 feed for Besigheim events — app-backbone ready.
...
import eventsRaw from "../../../public/data/besigheim/events-current.json";

const SITE     = "https://www.rausgucken.de";
const besigheim  = "besigheim";
const FEED_URL = `${SITE}/${besigheim}/feed.xml`;
const CITY_URL = `${SITE}/${besigheim}/`;
...
export async function GET() {
  ...
  const entries = events.map((ev) => {
    const url        = `${SITE}/${besigheim}/events/${esc(ev.slug)}/`;
    ...
    const ogImage    = `${SITE}/og/${besigheim}/${esc(ev.slug)}.jpg`;
...
<feed xmlns="http://www.w3.org/2005/Atom"
      ...
      xml:lang="de-DE">
  <id>${FEED_URL}</id>
  <title>Veranstaltungen in Besigheim | rausgucken.de</title>
  <subtitle>Aktuelle Events, Workshops und Ausstellungen in Besigheim – täglich aktualisiert.</subtitle>
