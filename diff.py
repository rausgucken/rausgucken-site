...
Takes  : output/ludwigsburg/04_schloss_transformed.json
         output/ludwigsburg/events-current.json  (previous run — may not exist)
Writes : output/ludwigsburg/04_schloss_diffed.json   (with is_new corrected)
...
INCOMING = ROOT / "output/ludwigsburg/04_schloss_transformed.json"
PREVIOUS = ROOT / "output/ludwigsburg/events-current.json"
...
def main():
    ...
    if PREVIOUS.exists():
        ...
        # Try to read previous scraped_at from meta
    ...
    for ev in incoming:
        ...
        if ev["is_new"]:
            added.append({"id": ev["id"], "title": ev["title"], "date_start": ev["date_start"]})

    # Removed = in previous but not in incoming
    if PREVIOUS.exists():
        for ev in prev_data:
            if ev["id"] not in incoming_ids:
                removed.append({"id": ev["id"], "title": ev["title"], "date_start": ev.get("date_start")})
