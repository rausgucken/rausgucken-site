 ...
 * Copy to functions/erdmannhausen/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   erdmannhausen        → URL slug, e.g. kornwestheim
 *   Erdmannhausen → display name, e.g. Kornwestheim
 *
 * Route: /erdmannhausen/events/*.ics
 *
 * Only activates for .ics requests. All other /erdmannhausen/events/* requests
...
export async function onRequest(context) {
  ...
  // /erdmannhausen/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/erdmannhausen/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/erdmannhausen/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/erdmannhausen/events/${ev.slug}">` +
