 ...
 * Copy to functions/oberstenfeld/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   oberstenfeld        → URL slug, e.g. kornwestheim
 *   Oberstenfeld → display name, e.g. Kornwestheim
 *
 * Route: /oberstenfeld/events/*.ics
 *
 * Only activates for .ics requests. All other /oberstenfeld/events/* requests
...
export async function onRequest(context) {
  ...
  // /oberstenfeld/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/oberstenfeld/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/oberstenfeld/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/oberstenfeld/events/${ev.slug}">` +
