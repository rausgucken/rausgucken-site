 ...
 * Copy to functions/murr/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   murr        → URL slug, e.g. kornwestheim
 *   Murr → display name, e.g. Kornwestheim
 *
 * Route: /murr/events/*.ics
 *
 * Only activates for .ics requests. All other /murr/events/* requests
...
export async function onRequest(context) {
  ...
  // /murr/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/murr/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/murr/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/murr/events/${ev.slug}">` +
