 ...
 * Route: /bietigheim/events/*.ics
 *
 * Only activates for .ics requests. All other /bietigheim/events/* requests
...
export async function onRequest(context) {
  ...
  // /bietigheim/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/bietigheim/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/bietigheim/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/bietigheim/events/${ev.slug}">` +
