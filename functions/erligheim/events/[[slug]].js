 ...
 * Copy to functions/erligheim/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   erligheim        → URL slug, e.g. kornwestheim
 *   Erligheim → display name, e.g. Kornwestheim
 *
 * Route: /erligheim/events/*.ics
 *
 * Only activates for .ics requests. All other /erligheim/events/* requests
...
export async function onRequest(context) {
  ...
  // /erligheim/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/erligheim/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/erligheim/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/erligheim/events/${ev.slug}">` +
