 ...
 * Route: /remseck/events/*.ics
 *
 * Only activates for .ics requests. All other /remseck/events/* requests
...
export async function onRequest(context) {
  ...
  // /remseck/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/remseck/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/remseck/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/remseck/events/${ev.slug}">` +
