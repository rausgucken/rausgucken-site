 ...
 * Copy to functions/oberriexingen/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   oberriexingen        → URL slug, e.g. kornwestheim
 *   Oberriexingen → display name, e.g. Kornwestheim
 *
 * Route: /oberriexingen/events/*.ics
 *
 * Only activates for .ics requests. All other /oberriexingen/events/* requests
...
export async function onRequest(context) {
  ...
  // /oberriexingen/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/oberriexingen/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/oberriexingen/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/oberriexingen/events/${ev.slug}">` +
