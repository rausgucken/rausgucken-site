 ...
 * Copy to functions/pleidelsheim/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   pleidelsheim        → URL slug, e.g. kornwestheim
 *   Pleidelsheim → display name, e.g. Kornwestheim
 *
 * Route: /pleidelsheim/events/*.ics
 *
 * Only activates for .ics requests. All other /pleidelsheim/events/* requests
...
export async function onRequest(context) {
  ...
  // /pleidelsheim/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/pleidelsheim/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/pleidelsheim/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/pleidelsheim/events/${ev.slug}">` +
