 ...
 * Copy to functions/ditzingen/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   ditzingen        → URL slug, e.g. kornwestheim
 *   Ditzingen → display name, e.g. Kornwestheim
 *
 * Route: /ditzingen/events/*.ics
 *
 * Only activates for .ics requests. All other /ditzingen/events/* requests
...
export async function onRequest(context) {
  ...
  // /ditzingen/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/ditzingen/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/ditzingen/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/ditzingen/events/${ev.slug}">` +
