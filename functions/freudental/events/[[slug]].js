 ...
 * Copy to functions/freudental/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   freudental        → URL slug, e.g. kornwestheim
 *   Freudental → display name, e.g. Kornwestheim
 *
 * Route: /freudental/events/*.ics
 *
 * Only activates for .ics requests. All other /freudental/events/* requests
...
export async function onRequest(context) {
  ...
  // /freudental/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/freudental/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/freudental/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/freudental/events/${ev.slug}">` +
