 ...
 * Route: /markgroeningen/events/*.ics
...
export async function onRequest(context) {
  ...
  let events;
  try {
    const dataUrl = new URL('/data/markgroeningen/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/markgroeningen/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);
  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(`X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/markgroeningen/events/${ev.slug}">Auf rausgucken.de ansehen</a>`);
