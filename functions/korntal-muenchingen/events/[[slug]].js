 ...
 * Copy to functions/korntal-muenchingen/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   korntal-muenchingen        → URL slug, e.g. kornwestheim
 *   Korntal-Münchingen → display name, e.g. Kornwestheim
 *
 * Route: /korntal-muenchingen/events/*.ics
 *
 * Only activates for .ics requests. All other /korntal-muenchingen/events/* requests
...
export async function onRequest(context) {
  ...
  // /korntal-muenchingen/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/korntal-muenchingen/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/korntal-muenchingen/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/korntal-muenchingen/events/${ev.slug}">` +
