 ...
 * Copy to functions/benningen/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   benningen        → URL slug, e.g. kornwestheim
 *   Benningen am Neckar → display name, e.g. Kornwestheim
 *
 * Route: /benningen/events/*.ics
 *
 * Only activates for .ics requests. All other /benningen/events/* requests
...
export async function onRequest(context) {
  ...
  // /benningen/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/benningen/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/benningen/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/benningen/events/${ev.slug}">` +
