 ...
 * Copy to functions/loechgau/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   loechgau        → URL slug, e.g. kornwestheim
 *   Löchgau → display name, e.g. Kornwestheim
 *
 * Route: /loechgau/events/*.ics
 *
 * Only activates for .ics requests. All other /loechgau/events/* requests
...
export async function onRequest(context) {
  ...
  // /loechgau/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/loechgau/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/loechgau/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/loechgau/events/${ev.slug}">` +
