 ...
 * Copy to functions/CITY_ID/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   CITY_ID        → URL slug, e.g. kornwestheim
 *   CITY_NAME_FULL → display name, e.g. Kornwestheim
 *
 * Route: /CITY_ID/events/*.ics
 *
 * Only activates for .ics requests. All other /CITY_ID/events/* requests
...
export async function onRequest(context) {
  ...
  // /CITY_ID/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/CITY_ID/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/CITY_ID/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/CITY_ID/events/${ev.slug}">` +
