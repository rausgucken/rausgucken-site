 ...
 * Route: /ludwigsburg/events/*.ics
 *
 * Only activates for .ics requests. All other /ludwigsburg/events/* requests
...
export async function onRequest(context) {
  ...
  // /ludwigsburg/events/some-event-slug.ics -> some-event-slug
  const slug = url.pathname.replace(/\.ics$/, '').split('/').filter(Boolean).pop();
  if (!slug) {
    return new Response('Not found', { status: 404 });
  }

  // Load events via ASSETS binding (same pattern as _temporal.js)
  let events;
  try {
    const dataUrl = new URL('/data/ludwigsburg/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  if (ev.time) {
    // Parse start time from "14:00 - 16:00 Uhr" or "14:00 Uhr"
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/ludwigsburg/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/ludwigsburg/events/${ev.slug}">` +
...
/** Date → iCal UTC stamp: 20260509T120000Z */
function utcStamp(d) {
  return d.toISOString().replace(/[-:]/g, '').replace(/\.\d{3}/, '');
}

/** Add N hours to YYYYMMDDTHHMMSS string */
