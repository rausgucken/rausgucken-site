 ...
 * Copy to functions/steinheim/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   steinheim        → URL slug, e.g. kornwestheim
 *   Steinheim an der Murr → display name, e.g. Kornwestheim
 *
 * Route: /steinheim/events/*.ics
 *
 * Only activates for .ics requests. All other /steinheim/events/* requests
...
export async function onRequest(context) {
  ...
  // /steinheim/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/steinheim/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/steinheim/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/steinheim/events/${ev.slug}">` +
