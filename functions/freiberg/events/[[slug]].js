 ...
 * Route: /freiberg/events/*.ics
 *
 * Only activates for .ics requests. All other /freiberg/events/* requests
...
export async function onRequest(context) {
  ...
  // /freiberg/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/freiberg/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/freiberg/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/freiberg/events/${ev.slug}">` +
