 ...
 * Copy to functions/schwieberdingen/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   schwieberdingen        → URL slug, e.g. kornwestheim
 *   Schwieberdingen → display name, e.g. Kornwestheim
 *
 * Route: /schwieberdingen/events/*.ics
 *
 * Only activates for .ics requests. All other /schwieberdingen/events/* requests
...
export async function onRequest(context) {
  ...
  // /schwieberdingen/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/schwieberdingen/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/schwieberdingen/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/schwieberdingen/events/${ev.slug}">` +
