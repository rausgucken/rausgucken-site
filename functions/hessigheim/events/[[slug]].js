 ...
 * Copy to functions/hessigheim/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   hessigheim        → URL slug, e.g. kornwestheim
 *   Hessigheim → display name, e.g. Kornwestheim
 *
 * Route: /hessigheim/events/*.ics
 *
 * Only activates for .ics requests. All other /hessigheim/events/* requests
...
export async function onRequest(context) {
  ...
  // /hessigheim/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/hessigheim/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/hessigheim/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/hessigheim/events/${ev.slug}">` +
