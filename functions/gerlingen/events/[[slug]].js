 ...
 * Copy to functions/gerlingen/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   gerlingen        → URL slug, e.g. kornwestheim
 *   Gerlingen → display name, e.g. Kornwestheim
 *
 * Route: /gerlingen/events/*.ics
 *
 * Only activates for .ics requests. All other /gerlingen/events/* requests
...
export async function onRequest(context) {
  ...
  // /gerlingen/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/gerlingen/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/gerlingen/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/gerlingen/events/${ev.slug}">` +
