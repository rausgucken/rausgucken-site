 ...
 * Copy to functions/besigheim/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   besigheim        → URL slug, e.g. kornwestheim
 *   Besigheim → display name, e.g. Kornwestheim
 *
 * Route: /besigheim/events/*.ics
 *
 * Only activates for .ics requests. All other /besigheim/events/* requests
...
export async function onRequest(context) {
  ...
  // /besigheim/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/besigheim/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/besigheim/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/besigheim/events/${ev.slug}">` +
