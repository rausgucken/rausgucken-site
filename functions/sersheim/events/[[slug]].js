 ...
 * Copy to functions/sersheim/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   sersheim        → URL slug, e.g. kornwestheim
 *   Sersheim → display name, e.g. Kornwestheim
 *
 * Route: /sersheim/events/*.ics
 *
 * Only activates for .ics requests. All other /sersheim/events/* requests
...
export async function onRequest(context) {
  ...
  // /sersheim/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/sersheim/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/sersheim/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/sersheim/events/${ev.slug}">` +
