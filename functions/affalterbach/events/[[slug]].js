 ...
 * Copy to functions/affalterbach/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   affalterbach        → URL slug, e.g. kornwestheim
 *   Affalterbach → display name, e.g. Kornwestheim
 *
 * Route: /affalterbach/events/*.ics
 *
 * Only activates for .ics requests. All other /affalterbach/events/* requests
...
export async function onRequest(context) {
  ...
  // /affalterbach/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/affalterbach/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/affalterbach/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/affalterbach/events/${ev.slug}">` +
