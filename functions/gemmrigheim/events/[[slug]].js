 ...
 * Copy to functions/gemmrigheim/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   gemmrigheim        → URL slug, e.g. kornwestheim
 *   Gemmrigheim → display name, e.g. Kornwestheim
 *
 * Route: /gemmrigheim/events/*.ics
 *
 * Only activates for .ics requests. All other /gemmrigheim/events/* requests
...
export async function onRequest(context) {
  ...
  // /gemmrigheim/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/gemmrigheim/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/gemmrigheim/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/gemmrigheim/events/${ev.slug}">` +
