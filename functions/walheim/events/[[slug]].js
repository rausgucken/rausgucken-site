 ...
 * Copy to functions/walheim/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   walheim        → URL slug, e.g. kornwestheim
 *   Walheim → display name, e.g. Kornwestheim
 *
 * Route: /walheim/events/*.ics
 *
 * Only activates for .ics requests. All other /walheim/events/* requests
...
export async function onRequest(context) {
  ...
  // /walheim/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/walheim/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/walheim/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/walheim/events/${ev.slug}">` +
