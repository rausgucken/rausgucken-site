 ...
 * Route: /tamm/events/*.ics
 *
 * Only activates for .ics requests. All other /tamm/events/* requests
...
export async function onRequest(context) {
  ...
  // /tamm/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/tamm/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/tamm/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/tamm/events/${ev.slug}">` +
