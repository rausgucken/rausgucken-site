 ...
 * Copy to functions/boennigheim/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   boennigheim        → URL slug, e.g. kornwestheim
 *   Bönnigheim → display name, e.g. Kornwestheim
 *
 * Route: /boennigheim/events/*.ics
 *
 * Only activates for .ics requests. All other /boennigheim/events/* requests
...
export async function onRequest(context) {
  ...
  // /boennigheim/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/boennigheim/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/boennigheim/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/boennigheim/events/${ev.slug}">` +
