 ...
 * Copy to functions/mundelsheim/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   mundelsheim        → URL slug, e.g. kornwestheim
 *   Mundelsheim → display name, e.g. Kornwestheim
 *
 * Route: /mundelsheim/events/*.ics
 *
 * Only activates for .ics requests. All other /mundelsheim/events/* requests
...
export async function onRequest(context) {
  ...
  // /mundelsheim/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/mundelsheim/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/mundelsheim/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/mundelsheim/events/${ev.slug}">` +
