 ...
 * Copy to functions/hemmingen/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   hemmingen        → URL slug, e.g. kornwestheim
 *   Hemmingen → display name, e.g. Kornwestheim
 *
 * Route: /hemmingen/events/*.ics
 *
 * Only activates for .ics requests. All other /hemmingen/events/* requests
...
export async function onRequest(context) {
  ...
  // /hemmingen/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/hemmingen/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/hemmingen/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/hemmingen/events/${ev.slug}">` +
