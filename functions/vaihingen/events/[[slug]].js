 ...
 * Copy to functions/vaihingen/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   vaihingen        → URL slug, e.g. kornwestheim
 *   Vaihingen an der Enz → display name, e.g. Kornwestheim
 *
 * Route: /vaihingen/events/*.ics
 *
 * Only activates for .ics requests. All other /vaihingen/events/* requests
...
export async function onRequest(context) {
  ...
  // /vaihingen/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/vaihingen/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/vaihingen/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/vaihingen/events/${ev.slug}">` +
