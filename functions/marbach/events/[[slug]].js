 ...
 * Copy to functions/marbach/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   marbach        → URL slug, e.g. kornwestheim
 *   Marbach am Neckar → display name, e.g. Kornwestheim
 *
 * Route: /marbach/events/*.ics
 *
 * Only activates for .ics requests. All other /marbach/events/* requests
...
export async function onRequest(context) {
  ...
  // /marbach/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/marbach/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/marbach/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/marbach/events/${ev.slug}">` +
