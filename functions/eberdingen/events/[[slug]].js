 ...
 * Copy to functions/eberdingen/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   eberdingen        → URL slug, e.g. kornwestheim
 *   Eberdingen → display name, e.g. Kornwestheim
 *
 * Route: /eberdingen/events/*.ics
 *
 * Only activates for .ics requests. All other /eberdingen/events/* requests
...
export async function onRequest(context) {
  ...
  // /eberdingen/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/eberdingen/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/eberdingen/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/eberdingen/events/${ev.slug}">` +
