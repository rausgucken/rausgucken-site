 ...
 * Copy to functions/grossbottwar/events/[[slug]].js when adding a new city.
 * Replace all tokens before use:
 *   grossbottwar        → URL slug, e.g. kornwestheim
 *   Großbottwar → display name, e.g. Kornwestheim
 *
 * Route: /grossbottwar/events/*.ics
 *
 * Only activates for .ics requests. All other /grossbottwar/events/* requests
...
export async function onRequest(context) {
  ...
  // /grossbottwar/events/some-event-slug.ics -> some-event-slug
  ...
  let events;
  try {
    const dataUrl = new URL('/data/grossbottwar/events-current.json', url.origin);
...
function buildICS(ev) {
  ...
  const eventUrl = ev.canonical_url || `https://www.rausgucken.de/grossbottwar/events/${ev.slug}`;
  lines.push(`URL:${eventUrl}`);

  lines.push(`ORGANIZER;CN=${escICS(orgName)}:mailto:noreply@rausgucken.de`);
  lines.push(
    `X-ALT-DESC;FMTTYPE=text/html:<a href="${site}/grossbottwar/events/${ev.slug}">` +
